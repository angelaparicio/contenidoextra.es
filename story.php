...
//Apaño cutre para los enlaces de twitter. Debería hacerse por htaccess
if ( strpos($_SERVER['REQUEST_URI'], 'utm_medium=twitter') > 0 ) {
	$story_id = str_replace( array('?utm_source=dlvr.it&utm_medium=twitter', '/story/'), array('', ''), $_SERVER['REQUEST_URI']);
	$link = Link::from_db($story_id);
...
if (!isset($_REQUEST['id']) && !empty($_SERVER['ORIG_PATH_INFO'])) {
	$url_args = preg_split('/\/+/', $_SERVER['ORIG_PATH_INFO'],  3, PREG_SPLIT_NO_EMPTY);
