...
/* Generated from /homepages/9/d377857919/htdocs/meneame/www/templates/story/link_sneak.html */
function haanga_b11ea5f321e4b80e116de98287579bb9780b371b($vars, $return=FALSE, $blocks=array())
{
    ...
    echo '<div class="mini-sneaker"> <fieldset> 
...
var sneak_base_url = \'http://'.$globals['server_name'].$globals['base_url'].'backend/link_sneaker.php\';
...
function received_data(data) {
	...
	if (requests > max_requests) {
		if ( !confirm(\''._('Fisgón: ¿desea continuar conectado?').'\') ) {
			return;
		}
		requests = 0;
		next_update = 100;
...
function to_html(data) {
	...
	if (data.type == \'vote\')
		if (data.status == \''._('publicada').'\')
			html += \'<div class="mini-sneaker-type"><img src="\'+base_url+\'img/common/sneak-vote-published01.png" width="21" height="17" alt="'._('voto').'" title="'._('voto').'" /><\\/div>\';
		else
			html += \'<div class="mini-sneaker-type"><img src="\'+base_url+\'img/common/sneak-vote01.png" width="21" height="17" alt="'._('voto').'" title="'._('voto').'" /><\\/div>\';
	else if (data.type == \'problem\')
		html += \'<div class="mini-sneaker-type"><img src="\'+base_url+\'img/common/sneak-problem01.png" width="21" height="17" alt="'._('problema').'" title="'._('problema').'" /><\\/div>\';
	else if (data.type == \'comment\') {
		html += \'<div class="mini-sneaker-type"><img src="\'+base_url+\'img/common/sneak-comment01.png" width="21" height="17" alt="'._('comentario').'" class="tooltip c:\'+data.id+\'"><\\/div>\';
	} else
...
//]]>
 ...
 <div class="mini-sneaker-item"> <div class="mini-sneaker-title"> <div class="mini-sneaker-ts"><strong>'._('hora').'</strong></div> <div class="mini-sneaker-type"><strong>'._('acción').'</strong></div> <div class="mini-sneaker-votes"><strong><abbr title="'._('votos').'">'._('me').'</abbr>/<abbr title="'._('comentarios').'">'._('co').'</abbr></strong></div> <div class="mini-sneaker-who">&nbsp;<strong>'._('quién/qué').'</strong></div> <div class="mini-sneaker-status"><strong>'._('estado').'</strong></div> </div> </div> ';
    for ($i = 0; $i <= 9; $i += 1) {
        echo ' <div id="sneaker-'.$i.'" class="mini-sneaker-item">&nbsp;</div> ';
    }
    echo ' </fieldset> </div> <script type="text/javascript">$(function(){start_link_sneak()});</script> ';
    if ($return == TRUE) {
        return ob_get_clean();
    }
}