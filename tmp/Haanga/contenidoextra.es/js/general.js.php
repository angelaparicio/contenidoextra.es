...
/* Generated from /homepages/9/d377857919/htdocs/meneame/www/templates/js/general.js */
function haanga_a250639d3e2ccf9cdd3c81fd3833845f1f9f6358($vars, $return=FALSE, $blocks=array())
{
...
function menealo_comment(user, id, value)
{
	...
	$.getJSON(url,
		 function(data) {
			if (data.error) {
				mDialog.notify("'._('Error:').' "+data.error, 5);
				return false;
			} else {
...
function parseLinkAnswer (id, link) {
	...
	if (link.error || id != link.id) {
		disable_vote_link(id, -1, "'._('grr...').'", \'\');
		mDialog.notify("'._('Error:').' "+link.error, 5);
		return false;
	}
...
function report_problem(frm, user, id) {
	if (frm.ratings.value == 0)
		return;
	mDialog.confirm("'._('¿desea votar').' <em>" + frm.ratings.options[frm.ratings.selectedIndex].text +"</em>?",
...
function fancybox_gallery(type, user, link) {
	if (! user_id > 0) {
		mDialog.notify(\''._('Debe estar autentificado para visualizar imágenes').'\', 5);
		return;
	}
...
var mDialog = new function() {
	...
	this.init = function() {
		...
		this.btYes = $("<button>").attr({ id: \'mDialog_yes\' }).text("'._('Sí').'");
		this.btNo = $("<button>").attr({ id: \'mDialog_no\' }).text("'._('No').'");
		this.btOk = $("<button>").attr({ id: \'mDialog_ok\' }).text("'._('Vale').'");
		this.btCancel = $("<button>").attr({ id: \'mDialog_ok\' }).text("'._('Cancelar').'");
...
function show_total_answers(type, id, answers) {
	...
	$(dom_id).siblings(".comment-meta").children(".comment-votes-info").append(\'<a href="javascript:show_answers(\\\'\'+type+\'\\\',\'+id+\')" title="\'+answers+\' '._('respuestas').'"><img style="margin-right:0px" src="'.$globals['base_static'].'img/common/replies-01.png" width="16" height="14"/><span class="counter answers">\'+answers+\'</span></a>\');
}
	...
	var m = {
		...
		traverse: function(files, area) {
			...
			if (typeof files !== "undefined") {
				...
				mDialog.notify("'._('formato no reconocido').'", 5);
			}
		},
			...
			if (files != undefined) {
				for (var i = 0; i < files.length; i++) {
					...
					if (typeof FileReader === "undefined" || !(/image/i).test(files[i].type)) {
						mDialog.notify("'._('sólo se admiten imágenes').'", 5);
						return false;
					}
					if (files[i].fileSize > s.maxsize) {
						mDialog.notify("'._('tamaño máximo excedido').'" + ":<br/>" + files[i].fileSize + " > " + s.maxsize + " bytes", 5);
						return false;
					}
				}
				return true;
