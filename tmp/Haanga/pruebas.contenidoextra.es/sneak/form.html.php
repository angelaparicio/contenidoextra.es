...
/* Generated from /homepages/9/d377857919/htdocs/meneame/dev/templates/sneak/form.html */
function haanga_728e4777c27166f439c67d7307f3acbb7ef36b35($vars, $return=FALSE, $blocks=array())
{
    ...
    if (!$globals['sneak_telnet']) {
        echo ' <img src="'.$globals['base_static'].'img/common/sneak-vote01.png" width="21" height="17" title="'._('votos').'" alt="'._('votos').'" /> ';
    } else {
        echo ' [+]'._('votos').' ';
    }
    ...
    echo ' </label> ';
    if ($globals['sneak_telnet']) {
        echo ' &nbsp;[<a href="sneak.php" title="'._('fisgona').'">'._('fisgona').'</a>]<br/> ';
    }
    ...
    if ($current_user->user_id > 0) {
        echo ' <form name="chat_form" onsubmit="return send_chat(this);" id="sneak-chat"> '._('mensaje').': <input type="text" name="comment" id="comment-input" class="droparea" value="" size="80" maxlength="230" autocomplete="off" style="font-size:11pt;min-height:12pt;"/>&nbsp;<input type="submit" value="'._('enviar').'" class="button"/> ';
        if (!$globals['sneak_telnet']) {
            echo ' <div class="droparea_info" style="float:right"></div> ';
        }
        echo ' </form> <script type="text/javascript"> $(\'#sneak-chat\').droparea({maxsize: '.$globals['media_max_size'].', show_thumb: false, hide_delay: 100, complete: function (r) { if (typeof r.url != \'undefined\') { input = $(\'#comment-input\'); input.val(input.val() + " " + r.url); input.focus(); } } }); </script> ';
    }
    echo ' </div> <div id="singlewrap"> <div class="sneaker-item"> <div class="sneaker-title"> <div class="sneaker-ts"><strong>'._('hora').'</strong></div> <div class="sneaker-type"><strong>'._('acción').'</strong></div> <div class="sneaker-votes"><strong><abbr title="'._('votos').'">me</abbr>/<abbr title="'._('comentarios').'">co</abbr></strong></div> <div class="sneaker-story">&nbsp;<strong>'._('noticia').'</strong></div> <div class="sneaker-who">&nbsp;<strong>'._('quién/qué').'</strong></div> <div class="sneaker-status"><strong>'._('estado').'</strong></div> </div> </div> <div id="items"> ';
