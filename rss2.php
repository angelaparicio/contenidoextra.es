...
if(!empty($_REQUEST['time'])) {
	...
	$last_modified = time();
	$title = _('contenido Extra').': '.sprintf(_('más votadas en %s'), txt_time_diff($from));
	...
	$title = _('contenido Extra').': '.sprintf(_('favoritas de %s'), $user_login);
	...
	$user_login = $db->get_var("select user_login from users where user_id=$user_id");
	$title = _('contenido Extra').': '.sprintf(_('votadas por %s'), $user_login);
	$globals['redirect_feedburner'] = false;
	...
	$user_login = $db->get_var("select user_login from users where user_id=$user_id");
	$title = _('contenido Extra').': '.sprintf(_('amigos de %s'), $user_login);
	$globals['redirect_feedburner'] = false;
	...
	$user_login = $db->get_var("select user_login from users where user_id=$user_id");
	$title = _('contenido Extra').': '.sprintf(_('noticias de %s'), $user_login);
	$globals['redirect_feedburner'] = false;
} else {
	/////
	// All the others
	...
	switch ($status) {
		case 'published':
			...
			$link_date = 'date';
			$title = _('contenido Extra').': '._('publicadas');
			break;
		case 'queued':
			$title = _('contenido Extra').': '._('en cola');
			$order_field = 'link_date';
			$link_date = "date";
			$home = "/shakeit.php";
		...
		case 'all_local':
		default:
			$title = _('contenido Extra').': '._('todas');
			$order_field = 'link_date';
			$link_date = "date";
			break;
	}
	...
	if($_REQUEST['q']) {
		...
		$title = _('contenido Extra') . ": " . htmlspecialchars(strip_tags($_REQUEST['q']));
	}
